...
//The half-life of caffeine is the time it takes for half of the caffeine in your body to be metabolized and eliminated. 
////On average, the half-life of caffeine is about 3 to 5 hours,
//but it can vary from person to person and even from one day to another.
//For example, if you consume 200 mg of caffeine, after 5 hours, you’ll have about 100 mg left in your system. 
//After another 5 hours, it reduces to 50 mg, and so on.
const halfLifeHours = 5


export function calculateCurrentCaffeineLevel(historyData) {
    const currentTime = Date.now()
    const halfLife = halfLifeHours * 60 * 60 * 1000 // = 18.000.000 milisecods which is 5 hours in milliseconds
    const maxAge = 48 * 60 * 60 * 1000 // = 172.000.000 48 hours in milliseconds

    let totalCaffeine = 0

    for (const [timestamp, entry] of Object.entries(historyData)) {
        const timeElapsed = currentTime - parseInt(timestamp) // now data - '1727575199865' for instance
...
export function getTopThreeCoffees(historyData) {
    ...
    // add any coffee given as an input for historyData
    // if we give coffeeConsumptionHistory as the input data it will add all 43 cofees
